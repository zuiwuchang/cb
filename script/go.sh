...
if [[ "$debug" == 1 ]];then
    ...
    args=(
        go build 
        -ldflags "\"-s -w -X 'github.com/zuiwuchang/cb/version.Version=$Version' -X 'github.com/zuiwuchang/cb/version.Date=$date' -X 'github.com/zuiwuchang/cb/version.Commit=$commit'\""
        -o "bin/$target"
    )
fi
